...
function rungsiaosp(){
	...
	if [ "$androidVersion" = "8.0" ];then
		...
		x-terminal-emulator -T $name"_CTS-ON-GSI_""${version[1]}" -e ./vts-tradefed run cts-on-gsi   ${serial_options[@]} --shard-count $countDevice --disable-reboot --skip-preconditions --module-arg CtsMediaTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5 \--module-arg CtsMediaStressTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5 \--module-arg CtsMediaBitstreamsTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5
