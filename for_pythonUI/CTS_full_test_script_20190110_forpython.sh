...
function CTS(){
	...
	if [ $androidVersion == "7.0" ]  || [ $androidVersion == "7.1" ] ;then
		...
		cd /3pl_report/cts/"${version[1]}"/$brand/$name/android-cts/tools
		x-terminal-emulator -T $name"_CTS_""${version[1]}" -e ./cts-tradefed run cts --shard-count $countDevice ${serial_options[@]}   --disable-reboot --skip-preconditions --module-arg CtsMediaTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5 \--module-arg CtsMediaStressTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5 \--module-arg CtsMediaBitstreamsTestCases:local-media-path:/CTS_tool/Media/android-cts-media-1.5
